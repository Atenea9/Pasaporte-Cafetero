...
// ─── Palette: amber honey + espresso + parchment ────────────────────
const T = {
  bg:          '#FBF7ED',
  card:        '#FFFDF8',
  dark:        '#2C1A0E',
...
const ROLES = [
  { num: '01', key: 'visitor',   credential: 'visitor@demo.com',   label: 'Visitante',  emoji: '🌿', c1: '#7B4A2A', c2: '#A0663C' },
  { num: '02', key: 'expositor', credential: 'expositor@demo.com', label: 'Expositor',  emoji: '🏪', c1: '#C8960C', c2: '#E8B820' },
  { num: '03', key: 'buyer',     credential: 'buyer@demo.com',     label: 'Comprador',  emoji: '🌍', c1: '#1565C0', c2: '#1976D2' },
];
...
function RoleCard({ role, loading, disabled, onPress }: {
  role: typeof ROLES[0]; loading: boolean; disabled: boolean; onPress: () => void;
}) {
  ...
  return (
    <Animated.View style={[s.card, { transform: [{ scale }], borderColor: role.c1 + '50' }]}>
      <TouchableOpacity onPress={onPress} onPressIn={pressIn} onPressOut={pressOut} activeOpacity={1} disabled={disabled} style={{ flex: 1 }}>
        <LinearGradient colors={[role.c1, role.c2]} style={s.cardGrad}>
          <View style={s.cardIconBox}>
            <Text style={s.cardIcon}>{role.emoji}</Text>
          </View>
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      >
        {/* Hero */}
        <View style={s.hero}>
          {/* Badge — the amber EDICIÓN 2026 pill */}
          <LinearGradient colors={[T.amberPale, '#FFF8DC']} style={s.heroBadge}>
            <Text style={s.heroBadgeText}>✦ EDICIÓN 2026 ✦</Text>
          </LinearGradient>

          {/* Logo Circle */}
          <LinearGradient colors={[T.coffeeDark, T.coffee, T.coffeeDark]} style={s.heroLogoWrap}>
...
const dd = StyleSheet.create({
  trigger:         { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: T.card, borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8, borderWidth: 1, borderColor: T.border },
  triggerText:     { color: T.dark, fontSize: 13, fontWeight: '800', letterSpacing: 1 },
  chevron:         { color: T.amber, fontSize: 10 },
  backdrop:        { flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'flex-start', alignItems: 'flex-end', paddingTop: 80, paddingRight: 20 },
...
const s = StyleSheet.create({
  ...
  amberDot:        { width: 8, height: 8, borderRadius: 4, backgroundColor: T.amber, opacity: 0.6 },
  scroll:          { flexGrow: 1, paddingHorizontal: 22, paddingTop: 110, paddingBottom: 40 },

  hero:            { alignItems: 'center', marginBottom: 32 },

  // Badge — amber honey pill
  heroBadge:       { borderRadius: 30, paddingHorizontal: 20, paddingVertical: 7, borderWidth: 1, borderColor: T.border, marginBottom: 22, shadowColor: T.amber, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 3 },
  heroBadgeText:   { color: T.amberDark, fontSize: 10, fontWeight: '900', letterSpacing: 4 },

  // Logo circle — coffee brown
  heroLogoWrap:    { width: 90, height: 90, borderRadius: 45, alignItems: 'center', justifyContent: 'center', marginBottom: 20, shadowColor: T.coffeeDark, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.35, shadowRadius: 12, elevation: 7, borderWidth: 2, borderColor: T.amber + '50' },
  heroLogoEmoji:   { fontSize: 44 },

  heroTitle:       { fontSize: 42, fontWeight: '900', color: T.dark, letterSpacing: -1, lineHeight: 44, textAlign: 'center' },
  heroAmber:       { fontSize: 42, fontWeight: '900', color: T.amber, letterSpacing: -1, lineHeight: 46, textAlign: 'center', marginBottom: 20 },
  heroDivider:     { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 18, width: '65%' },
  heroDivLine:     { flex: 1, height: 1, backgroundColor: T.borderMed },
  heroDivIcon:     { width: 34, height: 34, resizeMode: 'contain' },
  heroFair:        { fontSize: 12, fontWeight: '700', color: T.body, letterSpacing: 0.5, textAlign: 'center', marginBottom: 4 },
  heroCity:        { fontSize: 20, fontWeight: '900', color: T.dark, letterSpacing: 3, textAlign: 'center', marginBottom: 4 },
  heroYear:        { fontSize: 11, color: T.muted, letterSpacing: 2, textAlign: 'center' },
  ...
  sectionText:     { fontSize: 9, fontWeight: '900', color: T.muted, letterSpacing: 2.5 },

  cards:           { gap: 12, marginBottom: 24 },
  card:            { borderRadius: 18, overflow: 'hidden', borderWidth: 1, shadowColor: T.dark, shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.12, shadowRadius: 8, elevation: 4 },
  cardGrad:        { flexDirection: 'row', alignItems: 'center', paddingVertical: 20, paddingHorizontal: 20, gap: 14 },
  cardIconBox:     { width: 50, height: 50, borderRadius: 25, backgroundColor: 'rgba(255,255,255,0.22)', alignItems: 'center', justifyContent: 'center' },
  cardIcon:        { fontSize: 26 },
  cardBody:        { flex: 1 },
  cardTitle:       { fontSize: 20, fontWeight: '900', color: '#FFF', letterSpacing: 0.8 },
  cardNum:         { fontSize: 11, color: 'rgba(255,255,255,0.6)', marginTop: 2, letterSpacing: 2 },
